...
void USettingsEntryWidget::InitializeSetting(const FText& InDisplayName, ESettingWidgetType InWidgetType,
                                             const FString& InCVarName, EDisplaySettingType InDisplayType)
{
...
{
	FString SelecterString = SelectedItem.ToString();
	
	if (DisplayType == EDisplaySettingType::DisplayMode)
	{
		int32 Index = DisplayModeOptions.Find(SelecterString);	
		switch (Index)
		{
	...
	{
		int32 XPos = SelecterString.Find(TEXT("x"));
		FString WidthStr = SelecterString.Left(XPos);
		FString HeightStr = SelecterString.Mid(XPos + 1);
