...
{
	// Audio Cvars
	IConsoleManager::Get().RegisterConsoleVariable(TEXT("au.MasterVolume"), 1.0f, TEXT("Master volume setting"), ECVF_Default);
	IConsoleManager::Get().RegisterConsoleVariable(TEXT("au.MusicVolume"), 1.0f, TEXT("Music volume setting"), ECVF_Default);
	IConsoleManager::Get().RegisterConsoleVariable(TEXT("au.SFXVolume"), 1.0f, TEXT("SFX volume setting"), ECVF_Default);
	IConsoleManager::Get().RegisterConsoleVariable(TEXT("au.VoiceVolume"), 1.0f, TEXT("Voice volume setting"), ECVF_Default);
	
	// Input CVars
	IConsoleManager::Get().RegisterConsoleVariable(TEXT("input.MouseSensitivity"), 1.0f, TEXT("Mouse sensitivity multiplier"), ECVF_Default);
}
...
void USettingsWidget::LoadSettings(const FString& SlotName, bool bIsAudio)
{
	...
	UDefaultSaveGame* LoadedSave = Cast<UDefaultSaveGame>(UGameplayStatics::LoadGameFromSlot(SlotName, 0));
...
{
	UDefaultSaveGame* SaveGame = Cast<UDefaultSaveGame>(UGameplayStatics::CreateSaveGameObject(SaveGameObject));
	if (!SaveGame) return;
...
void USettingsWidget::ApplyAudioSettings(UDefaultSaveGame* SaveGame)
{
	...
	if (SFXSoundClass)
		UGameplayStatics::SetSoundMixClassOverride(GetWorld(), DefaultSoundMix, SFXSoundClass, SaveGame->VoiceVolume, 1.0f, 0.0f, true);
	if (VoiceSoundClass)
...
void USettingsWidget::OnSliderValueChanged(float Value, USoundClass* InSoundClass, const FString& CVarName)
{
	...
	if (IConsoleVariable* CVar = IConsoleManager::Get().FindConsoleVariable(*CVarName))
	{
		...
		else if (CVarName.Compare("input."))
		{
			ApplyInputSettings();
...
void USettingsWidget::AddResolutionSetting()
{
	if (VideoSettingsVB && SettingsEntryWidgetClass)
	{
		...
		if (ResolutionWidget)
		{
			ResolutionWidget->InitializeSetting(FText::FromString("Resolution"),
				ESettingWidgetType::DropDown, TEXT(""), EDisplaySettingType::Resolution);
...
void USettingsWidget::AddQualitySetting(const FText& DisplayName, const FString& CVarName)
{
	if (VideoSettingsVB && SettingsEntryWidgetClass)
	{
		...
		if (QualityWidget)
		{
			QualityWidget->InitializeSetting(DisplayName, ESettingWidgetType::DoubleButton, CVarName);
			VideoSettingsVB->AddChild(QualityWidget);
		}
	}
}

void USettingsWidget::AddSliderSetting(UVerticalBox* TargetVerticalBox, const FText& DisplayName,
	...
	if (TargetVerticalBox && SettingsEntryWidgetClass)
	{
		...
		if (SliderWidget)
		{
			...
			if (CVarName.Compare("au."))
			{
				...
				if (CVarName == TEXT("au.MusicVolume"))
					SliderWidget->AssociatedSoundClass = MusicSoundClass;
				if (CVarName == TEXT("au.SFXVolume"))
					SliderWidget->AssociatedSoundClass = SFXSoundClass;
				if (CVarName == TEXT("au.VoiceVolume"))
					SliderWidget->AssociatedSoundClass = VoiceSoundClass;
			}
			
			SliderWidget->InitializeSetting(DisplayName, ESettingWidgetType::Slider, CVarName);
